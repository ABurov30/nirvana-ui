			...
			: function (target, source) {
					for (
						...
						for (var key in from2)
							hasOwnProperty3.call(from2, key) &&
								(to[key] = from2[key])
...
var require_react_dom_development = __commonJS({
    ...
    in`) === 0
			...
			function isAttributeNameSafe(attributeName) {
				return hasOwnProperty3.call(
					validatedAttributeNameCache,
					attributeName
				)
					? !0
					: hasOwnProperty3.call(
							illegalAttributeNameCache,
							attributeName
					  )
					? !1
...
Your code should look like: 
  const MyComponent = lazy(() => import('./MyComponent'))`,
			...
			function setDefaultValue(node2, type, value) {
				;(type !== 'number' ||
					node2.ownerDocument.activeElement !== node2) &&
					(value == null
...
Check the render method of \`` +
			...
			function getNearestMountedFiber(fiber) {
				...
				if (fiber.alternate) for (; node2.return; ) node2 = node2.return
					...
					do
						(node2 = nextNode),
							(node2.effectTag & (Placement | Hydrating)) !==
								NoEffect && (nearestMounted = node2.return),
...
`
			...
			function traverseEnterLeave(from2, to, fn, argFrom, argTo) {
				for (
					var common =
							from2 && to
								? getLowestCommonAncestor(from2, to)
								: null,
						pathFrom = [];
					!(!from2 || from2 === common);
			...
			function findFirstSuspended(row) {
				for (var node2 = row; node2 !== null; ) {
					if (node2.tag === SuspenseComponent) {
						...
						var didSuspend =
							(node2.effectTag & DidCapture) !== NoEffect
